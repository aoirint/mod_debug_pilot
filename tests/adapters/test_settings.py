...
def test_application_data_dir_prefers_packaged_then_windows_paths(tmp_path: Path) -> None:
    ...
    assert (
        application_data_dir(
            {"FLET_APP_STORAGE_DATA": str(tmp_path / "packaged"), "APPDATA": "ignored"},
            home=tmp_path,
        )
        == tmp_path / "packaged"
    )
    assert application_data_dir({"APPDATA": str(tmp_path)}, home=Path("ignored")) == (
        tmp_path / "ModDebugPilot"
    )
    assert application_data_dir({}, home=tmp_path) == (
...
def test_repository_round_trip_and_missing_file(tmp_path: Path) -> None:
    ...
    repository = JsonConfigRepository(tmp_path / "nested" / "settings.json")
...
def test_repository_rejects_corrupt_schema(
    ...
    with pytest.raises(ValidationError, match="Configuration is invalid") as raised:
        asyncio.run(JsonConfigRepository(path).load())
