...
def make_runtime(
    *,
    ...
    launcher = FakeLauncher(process=selected_process, ready=ready, error=launcher_error)
...
def test_save_isolation_recovery_and_fail_closed_cases(*, tmp_path: Path) -> None:
    ...
    def fail_debug_directory(path: Path, *args: object, **kwargs: object) -> None:  # noqa: PLR0917 -- keyword-only-exception: external mock ABI.
        if path == save:
...
def test_bootstrap_rejects_backup_and_rolls_back_copy_failure(*, tmp_path: Path) -> None:
    ...
    with pytest.raises(AgentRuntimeError, match="backup already"):
        BootstrapIsolation(game_directory=game, state_root=state).activate(profile=profile)
    (state / "bootstrap-backup").rmdir()
    original_copy = shutil.copy2

    def fail_second_copy(source: Path, destination: Path) -> Path:  # noqa: PLR0917 -- keyword-only-exception: shutil callback ABI.
...
def test_tasklist_probe_success_and_failure() -> None:
    ...
    async def run() -> None:
        ...
        with patch("asyncio.create_subprocess_exec", AsyncMock(return_value=process)):
            assert await TasklistProcessProbe().image_is_running(image_name="Lethal Company.exe")
            assert not await TasklistProcessProbe().image_is_running(image_name="Other.exe")
        process.returncode = 1
        with (
            patch("asyncio.create_subprocess_exec", AsyncMock(return_value=process)),
            pytest.raises(AgentRuntimeError, match="inspect"),
        ):
            await TasklistProcessProbe().image_is_running(image_name="Lethal Company.exe")
...
def test_runtime_install_launch_capture_stop_and_artifact(*, tmp_path: Path) -> None:
    ...
    async def run() -> None:
        runtime, launcher, config = make_runtime(tmp_path=tmp_path)
        draft = tmp_path / "draft"
        make_profile(root=draft)
        bundle = tmp_path / "profile.mdp-profile"
        ProfileWorkspace.create_bundle(
            profile=draft, profile_name="Profile", source_mods=(), destination=bundle
        )
        assert (
        ...
        stopped = await runtime.stop(instance_id=snapshot.instance_id)
...
def test_runtime_install_and_lookup_rejections(*, tmp_path: Path) -> None:
    ...
    async def run() -> None:
        ...
        for operation in (runtime.stop, runtime.capture):
            with pytest.raises(AgentRuntimeError, match="not found"):
                await operation(instance_id="missing")
        with pytest.raises(AgentRuntimeError, match="not found"):
            runtime.artifact(instance_id="missing", relative="x")
...
def test_runtime_normal_game_port_and_launch_failure(*, tmp_path: Path) -> None:
    ...
    async def run() -> None:
        runtime, _, config = make_runtime(tmp_path=tmp_path / "normal", running_normal_game=True)
        make_profile(root=config.data_root / "profiles/p")
        with pytest.raises(AgentRuntimeError, match="outside"):
            await runtime.launch(spec=InstanceSpec(name="x", profile_id="p"))

        runtime, _, config = make_runtime(tmp_path=tmp_path / "fail", launcher_error=True)
        make_profile(root=config.data_root / "profiles/p")
        with pytest.raises(OSError, match="launch"):
            await runtime.launch(spec=InstanceSpec(name="x", profile_id="p"))
        assert not config.save_directory.exists()

        runtime, _, config = make_runtime(tmp_path=tmp_path / "ports")
        make_profile(root=config.data_root / "profiles/p")
        first = await runtime.launch(
            spec=InstanceSpec(name="x", profile_id="p", debugger_port=55555)
        )
        with pytest.raises(AgentRuntimeError, match="port"):
            await runtime.launch(spec=InstanceSpec(name="y", profile_id="p", debugger_port=55555))
        make_profile(root=config.data_root / "profiles/q", bootstrap=b"different")
        with pytest.raises(AgentRuntimeError, match="identical"):
            await runtime.launch(spec=InstanceSpec(name="z", profile_id="q", debugger_port=55557))
        second = await runtime.launch(
            spec=InstanceSpec(name="y", profile_id="p", debugger_port=55556)
        )
        assert len(await runtime.list_instances()) == len((first, second))
        await runtime.stop(instance_id=second.instance_id)
        await runtime.stop(instance_id=second.instance_id)
        await runtime.stop(instance_id=first.instance_id)
...
def test_runtime_redirect_failures_and_timeout(*, tmp_path: Path) -> None:
    ...
    async def run() -> None:
        ...
        runtime, _, config = make_runtime(tmp_path=tmp_path / "exited", process=exited, ready=False)
        make_profile(root=config.data_root / "profiles/p")
        ...
        silent = FakeProcess()
        runtime, _, config = make_runtime(tmp_path=tmp_path / "silent", process=silent, ready=False)
        make_profile(root=config.data_root / "profiles/p")
        with (
            patch(
        ...
        delayed = FakeProcess()
        runtime, _, config = make_runtime(
            tmp_path=tmp_path / "delayed", process=delayed, ready=False
        )
        make_profile(root=config.data_root / "profiles/p")
        with (
            patch(
            ...
            launched = await runtime.launch(spec=InstanceSpec(name="x", profile_id="p"))
        await runtime.stop(instance_id=launched.instance_id)
...
def test_runtime_natural_exit_shutdown_and_watcher_paths(*, tmp_path: Path) -> None:
    ...
    async def run() -> None:
        process = FakeProcess()
        runtime, _, config = make_runtime(tmp_path=tmp_path / "natural", process=process)
        make_profile(root=config.data_root / "profiles/p")
        snapshot = await runtime.launch(spec=InstanceSpec(name="x", profile_id="p"))
        process.code = 7
        ...
        runtime, _, config = make_runtime(tmp_path=tmp_path / "shutdown", process=process)
        make_profile(root=config.data_root / "profiles/p")
        await runtime.launch(spec=InstanceSpec(name="x", profile_id="p"))
        await runtime.shutdown()
        ...
        process = FakeProcess()
        runtime, _, config = make_runtime(tmp_path=tmp_path / "shutdown-success", process=process)
        make_profile(root=config.data_root / "profiles/p")
        running = await runtime.launch(spec=InstanceSpec(name="x", profile_id="p"))
        ...
        runtime, _, _ = make_runtime(tmp_path=tmp_path / "watch")
        with patch("mod_debug_pilot.infrastructure.agent_runtime.asyncio.sleep", AsyncMock()):
            await runtime._watch_instance(instance_id="missing")  # noqa: SLF001
        with patch(
        ...
        process = FakeProcess()
        runtime, _, config = make_runtime(tmp_path=tmp_path / "watch-exit", process=process)
        make_profile(root=config.data_root / "profiles/p")
        snapshot = await runtime.launch(spec=InstanceSpec(name="x", profile_id="p"))
        runtime._watchers[snapshot.instance_id].cancel()  # noqa: SLF001
        count = 0

        async def exit_after_poll(  # keyword-only-exception: asyncio callback ABI.
            _seconds: float,
        ) -> None:
            nonlocal count
            count += 1
        ...
        with patch(
            ...
            await runtime._watch_instance(instance_id=snapshot.instance_id)  # noqa: SLF001
...
def test_runtime_helpers_and_system_default(*, tmp_path: Path) -> None:
    ...
    assert _safe_identifier(value="profile-1")
    assert not _safe_identifier(value="")
    assert not _safe_identifier(value="x" * 65)
    assert not _safe_identifier(value="bad/name")
    root = tmp_path / "root"
    root.mkdir()
    assert _confined(root=root, relative="a.txt") == root / "a.txt"
    for relative in ("", "../x"):
        with pytest.raises(AgentRuntimeError):
            _confined(root=root, relative=relative)
    marker = root / "log"
    assert not _file_contains(path=marker, marker="x")
    marker.write_text("hello x", encoding="utf-8")
    assert _file_contains(path=marker, marker="x")
    with patch.object(Path, "read_text", side_effect=OSError):
        assert not _file_contains(path=marker, marker="x")
    config = make_config(tmp_path=tmp_path / "default")
    with patch("mod_debug_pilot.infrastructure.agent_runtime.sys.platform", "win32"):
        assert isinstance(RemoteAgentRuntime.system_default(config=config), RemoteAgentRuntime)
...
def test_runtime_recover_delegates_transactions(*, tmp_path: Path) -> None:
    ...
    async def run() -> None:
        runtime, _, _ = make_runtime(tmp_path=tmp_path)
...
def test_runtime_none_process_and_missing_artifact(*, tmp_path: Path) -> None:
    ...
    class NoneLauncher:
        async def launch(  # keyword-only-exception: external mock ABI.
    ...
    async def run() -> None:
        ...
        runtime, _, config = make_runtime(tmp_path=tmp_path / "artifact")
        make_profile(root=config.data_root / "profiles/p")
        snapshot = await runtime.launch(spec=InstanceSpec(name="x", profile_id="p"))
        with pytest.raises(AgentRuntimeError, match="Artifact"):
            runtime.artifact(instance_id=snapshot.instance_id, relative="missing")
        await runtime.stop(instance_id=snapshot.instance_id)
