...
class AppController:
    ...
    async def save_settings(self, values: dict[str, object]) -> bool:
        ...
        if self._closed or self._active_task is not None:
            return False
        self._commit(replace(self._state, phase=AppPhase.SAVING, message="Saving…"))
        try:
            config = await self._settings.save(values)
    ...
    async def _run_job(self, *, kind: JobKind, generation: int) -> None:
        try:
            result = await self._jobs.run(kind, config=self._state.config)
            if generation != self._generation or self._closed:
                return
            phase = phase_for_outcome(result.outcome)
            self._commit(
                replace(
                    self._state,
                    phase=phase,
