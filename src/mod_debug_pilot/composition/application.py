...
def compose_controller(*, data_dir: Path | None = None) -> AppController:
    ...
    resolved_data_dir = data_dir or application_data_dir(os.environ, home=Path.home())
    settings = SettingsService(JsonConfigRepository(resolved_data_dir / "settings.json"))
    jobs = JobService(LocalJobExecutor.system_default(), request_factory=SystemRequestFactory())
